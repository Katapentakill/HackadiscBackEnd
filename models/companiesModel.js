...
class Multicompany {
    ...
    static findById(db, main_company_id, main_company_name, callback) {
        db.get(`SELECT * FROM Multicompanies WHERE main_company_id = ? AND main_company_name = ?`, 
               [main_company_id, main_company_name], 
               (err, row) => {
            if (err) {
                console.error('Error al obtener compania por main_company_id y main_company_name:', err);
                callback(err, null);
            } else {
                callback(null, row);
            }
